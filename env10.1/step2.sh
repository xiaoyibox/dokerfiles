...
docker run -d -p 80:80 -m 256M --memory-swap 512M --net=env10 --ip=192.168.10.2 --restart=always --name env10_2_Nginx01 yi/centos7-ssh-tengine-local
echo 'Run env10_3_Redis01'
docker run -d  -m 256M --memory-swap 512M --net=env10 --ip=192.168.10.3 --restart=always --name env10_3_Redis01 yi/centos7-redis
echo 'Run env10_4_Mysql01'
docker run -d -p 3306:3306  -m 256M --memory-swap 512M  --net=env10 --ip=192.168.10.4 --restart=always --name env10_4_Mysql01 yi/centos7-mysql5.6.38
echo 'Run env10_5_Zookeeper01'
docker run -d  -m 256M --memory-swap 512M --net=env10 --ip=192.168.10.5 --restart=always --name env10_5_Zookeeper01 yi/centos7-zookeeper3.4.11
echo 'Run env10_6_Mq01'
docker run -d  -m 256M --memory-swap 512M --net=env10 --ip=192.168.10.6 --restart=always --name env10_6_Mq01 yi/centos7-activemq5.15.2
echo 'Run env10_7_Mycat01'
docker run -d  -m 256M --memory-swap 512M --net=env10 --ip=192.168.10.7 --restart=always --name env10_7_Mycat01 yi/centos7-mycat1.6

echo 'Run env10_201_confluence'
docker run -d -m 1024M --memory-swap 2048M --net=env10 --ip=192.168.10.201 --restart=always --name env10_201_confluence yi/centos7-confluence
echo 'Run env10_202_jira'
docker run -d -m 1024M --memory-swap 2048M --net=env10 --ip=192.168.10.202 --restart=always --name env10_202_jira yi/centos7-jira
